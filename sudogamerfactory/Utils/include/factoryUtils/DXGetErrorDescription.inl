...
// -------------------------------------------------------------
    ...
    CHK_ERR(D3D12_ERROR_DRIVER_VERSION_MISMATCH, "The specified cached PSO was created on a different driver version and cannot be reused on the c.")
    CHK_ERR(E_FAIL, "Attempted to create a device with the debug layer enabled and the layer is not installed.")
    CHK_ERR(E_INVALIDARG, "An invalid parameter was passed to the returning function.")
    CHK_ERR(E_OUTOFMEMORY, "Direct3D could not allocate sufficient memory to complete the call.")
    CHK_ERR(E_NOTIMPL, "The method call isn't implemented with the passed parameter combination.")
    CHK_ERR(S_FALSE, "Alternate success value, indicating a successful but nonstandard completion (the precise meaning depends on context)")
    CHK_ERR(S_OK, "No error occurred.")
